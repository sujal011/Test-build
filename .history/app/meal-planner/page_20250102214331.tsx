...
export default function MealPlannerPage({ className }: PageProps) {
  const router = useRouter()
  ...
  const handleInputChange = (field: keyof UserData, value: string | number | string[]) => {
    setUserData(prev => ({
      ...prev,
      [field]: value
    }))
  ...
  const calculateTDEE = (): number => {
    ...
    return bmr * (activityMultipliers[userData.activityLevel as keyof typeof activityMultipliers] || 1.2)
  }
  ...
  const handleGeneratePlan = async () => {
    ...
    try {
      // Calculate nutritional needs based on user data
      const tdee = calculateTDEE()
      const proteinNeeds = userData.weight * 2 // 2g per kg bodyweight
      const fatNeeds = (tdee * 0.25) / 9 // 25% of calories from fat
      const carbNeeds = (tdee - (proteinNeeds * 4 + fatNeeds * 9)) / 4

      const newNutritionData = [
        { name: 'Protein', value: Math.round(proteinNeeds) },
        { name: 'Carbs', value: Math.round(carbNeeds) },
        { name: 'Fat', value: Math.round(fatNeeds) }
      ]
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          ...
          <div className="bg-white p-6 rounded-lg shadow">
            <h2 className="text-xl font-semibold mb-4">Your Nutrition Profile</h2>
            
            <div className="mb-4">
              <p className="text-gray-600">BMI: {calculateBMI().toFixed(1)}</p>
              <p className="text-gray-600">Daily Calorie Need: {Math.round(calculateTDEE())} kcal</p>
            </div>
