...
export default function MealPlannerPage() {
  ...
  const fetchMealPlan = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const USER_ID = process.env.NEXT_PUBLIC_EDAMAM_USER_ID
      
      if (!APP_ID || !APP_KEY || !USER_ID) {
        throw new Error('Missing API configuration. Please check your environment variables.')
      }

      if (preferences.calories && (isNaN(Number(preferences.calories)) || Number(preferences.calories) < 0)) {
